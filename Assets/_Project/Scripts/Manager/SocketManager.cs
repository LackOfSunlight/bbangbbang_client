...
public class SocketManager : TCPSocketManagerBase<SocketManager>
{
    ...
    public void RegisterResponse(GamePacket gamePacket)
    {
        if (gamePacket == null)
        {
            Debug.LogError("GamePacket is null!");
            return;
        }

        if (gamePacket.PayloadCase != GamePacket.PayloadOneofCase.RegisterResponse)
        {
            Debug.LogError($"잘못된 패킷 도착: {gamePacket.PayloadCase}");
            return;
        }
        ...
        if (target == null)
        {
            ...
            switch (card.cardType)
            {
                case CardType.Call119:
                    target = new UserInfo { id = 0, nickname = "모든유저" };
                    targetText = target.nickname;
                    break;
                case CardType.MaturedSavings:
                    target = new UserInfo { id = 0, nickname = "은행원" };
                    targetText = target.nickname;
                    break;
                case CardType.WinLottery:
                    target = new UserInfo { id = 0, nickname = "복권방 사장" };
                    targetText = target.nickname;
                    break;
            }
        }
