...
public class PopupDeck : UIListBase<Card>
{
    ...
    public void OnUseCard()
    {
        var idx = uiPagingViewController.selectedIdx;
        CardDataSO card;
        GameManager.instance.selectedIndex = idx;

        if (SocketManager.instance.isConnected)
        {
            card = UserInfo.myInfo.handCards[idx];
            GameManager.instance.SelectedCard = card;
        }
        else
        {
            card = UserInfo.myInfo.OnUseCard(idx);
        }

        if (card.isTargetSelect || (UserInfo.myInfo.isSniper && card.cardType == CardType.Bbang && UserInfo.myInfo.isShotPossible))
        {
